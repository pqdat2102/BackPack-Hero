...
public class UpGrageBuffUI : MonoBehaviour
{
    ...
    [SerializeField] private PlayerResources playerResources; // Tham chiếu đến PlayerResources để lấy thông tin người chơi
    ...
    private void UpgradeRandomFaces(int numberOfFaces, int amount)
    {
        ...
        for (int i = 0; i < numberOfFaces && i < faceIndices.Count; i++)
        {
            ...
            string textValue = dice.faceTexts[faceIndex].text;
            int currentValue = int.Parse(textValue);
            currentValue += amount;
            dice.faceTexts[faceIndex].text = currentValue.ToString();
            //Debug.Log($"Mặt {faceIndex + 1} tăng giá trị lên {amount}. Giá trị mới: {currentValue}");
        }
        playerResources.SetWaitingForUpgrade(false);
    }
    ...
    private void UpgradeAllFaces(int amount)
    {
        ...
        {
            string textValue = dice.faceTexts[i].text;
            int currentValue = int.Parse(textValue);
            currentValue += amount;
